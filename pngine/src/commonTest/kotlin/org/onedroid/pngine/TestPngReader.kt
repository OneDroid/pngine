...
internal object TestPngReader {
    ...
    fun read(bytes: ByteArray): DecodedPng {
        ...
        while (pos < bytes.size) {
            ...
            val type = chunkType(bytes, pos + 4)
            val dataStart = pos + 8
            ...
            val actualCrc = crc32(bytes.copyOfRange(pos + 4, pos + 8), data)
        ...
        val raw = TestInflate.zlibDecompress(compressed)
    ...
    private fun chunkType(bytes: ByteArray, offset: Int): String =
        buildString { for (i in 0 until 4) append(bytes[offset + i].toInt().toChar()) }
